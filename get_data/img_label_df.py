...
def img_label_df(proj_dir, aimlab_dir, save_img_type):
    ...
    patids = []
    for patid in df['patid']:
        if patid not in fns:
            patids.append(patid)
    df_label = df[~df['patid'].isin(patids)]
    ...
    df = pd.merge(df_label, df_img, on='patid')
    print('total df size:', df.shape)
    print(df[0:20])
    df.to_csv(os.path.join(pro_data_dir, 'df_img_label.csv'), index=False)
    df.to_csv(os.path.join(aimlab_dir, 'df_img_label.csv'), index=False)
