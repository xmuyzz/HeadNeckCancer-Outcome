...
def crop(proj_dir):
    ...
    img_ids = []
    count = 0
    for img_dir in sorted(glob.glob(img_reg_dir + '/*nrrd')):
        img_id = img_dir.split('/')[-1].split('.')[0]
        for seg_dir in sorted(glob.glob(seg_reg_dir + '/*nrrd')):
            seg_id = seg_dir.split('/')[-1].split('.')[0]
            if img_id == seg_id:
                count += 1
                print(count)
...
def main():
    ...
    if do_combine_mask:
        combine_mask(proj_dir=proj_dir)
    if do_interpolate:
        interpolate_img(proj_dir=proj_dir)
    if do_register:
        registration(proj_dir=proj_dir)
    if do_crop:
        crop(proj_dir=proj_dir)
