...
class concordance_callback(tt.cb.MonitorMetrics):
    ...
    def on_epoch_end(self):
        ...
        if c_index >= best_c_index:
            self.model.save_model_weights(self.task_dir + '/models/weights_best_cindex.pt')
            print('best c-index model weights saved!')
        if c_index > self.target_c_index:
            self.model.save_model_weights(self.task_dir + '/models/weights_target_cindex.pt')
            print('target c-index model weights saved!')
        if va_loss <= best_va_loss:
            self.model.save_model_weights(self.task_dir + '/models/weights_best_loss.pt')
            print('best loss model weights saved!')

        # save logs (overwrite)
        np.save(self.task_dir + '/metrics/tr_losses.npy', tr_losses)
        np.save(self.task_dir + '/metrics/va_losses.npy', va_losses)
        np.save(self.task_dir + '/metrics/c_indexs.npy', self.c_indexs)
        np.save(self.task_dir + '/metrics/lrs.npy', self.lrs)
        ...
        if plot_loss:
            ...
            #log_path = self.log_dir + '/loss_' + strftime('%Y_%m_%d', localtime()) + '.png'
            plt.savefig(self.task_dir + '/loss.png', format='png', dpi=200)
            plt.close()
        ...
        if plot_cindex:
            ...
            plt.grid(True)
            plt.tight_layout(pad=1.08, h_pad=None, w_pad=None, rect=None)
            #log_path = self.log_dir + '/c_indx_' + strftime('%Y_%m_%d', localtime()) + '.png'
            plt.savefig(self.task_dir + '/c_index.png', format='png', dpi=200)
            plt.close()
        ...
        save_logger = True
        if save_logger:
            callback_logger(tr_log_path=self.tr_log_path, tr_loss=tr_loss, va_loss=va_loss, 
                lr=lr, c_index=c_index, best_c_index=best_c_index, epoch=epoch, epoch_time=epoch_time)
    ...
    def on_fit_end(self):
        super().on_epoch_end()
        self.model.save_net(self.task_dir + '/models/weights_final.pt')
