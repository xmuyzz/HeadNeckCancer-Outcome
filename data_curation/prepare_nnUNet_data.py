...
def prepare_nnUNet_data(proj_dir, data_dir, data_type):
    ...
    img_ts4_dir = data_dir + '/imagesTs4'
    seg_ts4_dir = data_dir + '/labelsTs4'
    img_ts6_dir = data_dir + '/imagesTs6'
    seg_ts6_dir = data_dir + '/labelsTs6'
    ...
    if not os.path.exists(img_ts4_dir):
        os.makedirs(img_ts4_dir)
    if not os.path.exists(seg_ts4_dir):
        os.makedirs(seg_ts4_dir)
    if not os.path.exists(img_ts6_dir):
        os.makedirs(img_ts6_dir)
    if not os.path.exists(seg_ts6_dir):
        os.makedirs(seg_ts6_dir)

    # get df
    if data_type == 'train':
        img_crop_dir = proj_dir + '/data/TOT/crop_img_160'
        seg_crop_dir = proj_dir + '/data/TOT/crop_seg_pn_160'
        ...
        img_crop_dir = '/mnt/kannlab_rfa/Zezhong/HeadNeck/Data/OPC2/crop_img_160'
        seg_crop_dir = '/mnt/kannlab_rfa/Zezhong/HeadNeck/Data/OPC2/crop_seg_p_n_160'
    elif data_type == 'test6':
        img_crop_dir = '/mnt/kannlab_rfa/Zezhong/HeadNeck/Data/MAASTRO/crop_img_160'
        seg_crop_dir = '/mnt/kannlab_rfa/Zezhong/HeadNeck/Data/MAASTRO/crop_seg_p_n_160'
    ...
    if data_type == 'train':
        ...
        df_ts2.to_csv(data_dir + '/output_files/df_ts2_pn.csv', index=False)
        ...
        df_noseg.to_csv(data_dir + '/output_files/no_seg_pn.csv', index=False)
    ...
    ### external test dataset: test2
        ...
        for i, (img_dir, seg_dir) in enumerate(zip(df['img_dir'], df['seg_dir'])):
            img_nn_id = 'TS4_' + str(f'{i:04}') + '_0000.nii.gz'
            seg_nn_id = 'TS4_' + str(f'{i:04}') + '.nii.gz'
            print(i, img_nn_id)
            img_save_dir = img_ts4_dir + '/' + img_nn_id
            seg_save_dir = seg_ts4_dir + '/' + seg_nn_id
            img = sitk.ReadImage(img_dir)
            sitk.WriteImage(img, img_save_dir)
            seg = sitk.ReadImage(seg_dir)
            sitk.WriteImage(seg, seg_save_dir)
        ...
        df.to_csv(data_dir + '/output_files/ts4_pn.csv', index=False)
        # img dataset without seg
        img_nn_ids = []
        for i, img_dir in enumerate(df_noseg['img_dir']):
            ...
            img_nn_ids.append(img_nn_id)
        df_noseg['img_nn_id'] = img_nn_ids
        df_noseg.to_csv(data_dir + '/output_files/df_no_seg_pn4.csv', index=False)
    ...
    ### external test dataset: test2
        ...
        seg_nn_ids = []
        for i, (img_dir, seg_dir) in enumerate(zip(df['img_dir'], df['seg_dir'])):
            img_nn_id = 'TS6_' + str(f'{i:04}') + '_0000.nii.gz'
            seg_nn_id = 'TS6_' + str(f'{i:04}') + '.nii.gz'
            print(i, img_nn_id)
            img_save_dir = img_ts6_dir + '/' + img_nn_id
            seg_save_dir = seg_ts6_dir + '/' + seg_nn_id
            img = sitk.ReadImage(img_dir)
            sitk.WriteImage(img, img_save_dir)
            seg = sitk.ReadImage(seg_dir)
            sitk.WriteImage(seg, seg_save_dir)
            ...
            seg_nn_ids.append(seg_nn_id)
        df['img_nn_id'], df['seg_nn_id'] = [img_nn_ids, seg_nn_ids]
        df.to_csv(data_dir + '/output_files/ts6_pn.csv', index=False)
