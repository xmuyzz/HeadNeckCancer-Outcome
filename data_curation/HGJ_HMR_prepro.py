...
def combine_mask(proj_dir, raw_data_dir):
    ...
    COMBINING MASKS 
    """
    raw_img_dir = proj_dir + '/TCIA/HGJ_HMR_data/raw_img'
    HGJ_seg_dir = raw_data_dir + '/HGJ_files/1_label_raw_HGJ_named'
    HMR_seg_dir = raw_data_dir + '/HMR_files/1_label_raw_HMR'
    seg_save_dir = proj_dir + '/TCIA/HGJ_HMR_data/raw_n_seg'
 
    # rename img file and transfer to lab drive
    HGJ_seg_dirs = [i for i in sorted(glob.glob(HGJ_seg_dir + '/*'))]
    HMR_seg_dirs = [i for i in sorted(glob.glob(HMR_seg_dir + '/*'))]
    raw_seg_dirs = HGJ_seg_dirs + HMR_seg_dirs
    #print(raw_seg_dirs)
    img_ids = []
    seg_namess = []
    for j, img_dir in enumerate(sorted(glob.glob(raw_img_dir + '/*nrrd'))):
        seg_names = []
        seg_dirs = []
        img_id = img_dir.split('/')[-1].split('.')[0]
        #print(img_id)
        for seg_folder in raw_seg_dirs:
            #print(folder)
            seg_id = seg_folder.split('-')[-2] + '_' + seg_folder.split('-')[-1]
            #print(seg_id)
            if seg_id == img_id:
                print('ID:', seg_id)
    ...
    fn = proj_dir + '/TCIA/HGJ_HMR_data/combined_seg_n.csv'
...
def crop(proj_dir, tumor_type, label_only):
    ...
    seg_p_reg_dir = os.path.join(proj_dir, 'TCIA/HGJ_HMR_data/reg_seg_p')
    seg_n_reg_dir = os.path.join(proj_dir, 'TCIA/HGJ_HMR_data/reg_seg_n')
    img_crop_dir = proj_dir + '/TCIA/HGJ_HMR_data/crop_img_160x160x64'
    seg_pn_crop_dir = proj_dir + '/TCIA/HGJ_HMR_data/crop_seg_pn'
    seg_p_crop_dir = proj_dir + '/TCIA/HGJ_HMR_data/crop_seg_p'
    seg_n_crop_dir = proj_dir + '/TCIA/HGJ_HMR_data/crop_seg_n_160x160x64'
